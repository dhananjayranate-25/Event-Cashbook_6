...
const API_URL = '';
    ...
    if (audioElem.paused) {
        audioElem.play().then(() => {
            ...
            btnText.innerText = 'Pause Mantra';
        }).catch(e => console.error('Audio play failed', e));
    } else {
        audioElem.pause();
        playIcon.style.display = 'inline-block';
        pauseIcon.style.display = 'none';
        btnText.innerText = 'Play Mantra';
    }
}
...
if (settings.bgAudioUrl) {
    ...
    if (audioElem && container) {
        ...
        // Try autoplaying on first user interaction if not already playing
        const playAudioOnInteract = () => {
            if (audioElem.paused) {
                audioElem.play().then(() => {
                    document.getElementById('audioIconPlay').style.display = 'none';
                    document.getElementById('audioIconPause').style.display = 'inline-block';
                    document.getElementById('audioBtnText').innerText = 'Pause Mantra';
                }).catch(e => {});
            }
        ...
        document.addEventListener('touchstart', playAudioOnInteract);
    }
}
