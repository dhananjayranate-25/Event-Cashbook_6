    ...
    function renderVisibleYearData(container) {
        ...
        for (let i = 0; i < entries.length; i++) {
            ...
            rows += '<tr><td>' + (i + 1) + '</td><td class="remark">' + e.name + '</td><td class="date">' + formatDate(e.date, true) + '</td><td><span class="mode-badge ' + modeClass + '">' + e.mode + '</span></td><td class="cash-in-cell">' + (ci ? '\u20B9' + ci.toFixed(2) : '-') + '</td><td class="cash-out-cell">' + (co ? '\u20B9' + co.toFixed(2) : '-') + '</td><td class="balance-cell">\u20B9' + runningBalance.toFixed(2) + '</td></tr>';
        }
        if (entries.length === 0) {
        ...
        html += '<div class="year-section-header"><h2>सन ' + year + ' - अहवाल</h2>';
        html += '</div>';

        html += '<div class="summary-cards">';
        html += '<div class="card card-cash-in"><div class="card-glass"><div class="card-header"><div class="card-icon-wrapper icon-green"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg></div><h3>एकूण जमा</h3></div><p class="amount">\u20B9' + totalIn.toFixed(2) + '</p></div></div>';
        html += '<div class="card card-cash-out"><div class="card-glass"><div class="card-header"><div class="card-icon-wrapper icon-red"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 1 0 7H6"/></svg></div><h3>एकूण खर्च</h3></div><p class="amount">\u20B9' + totalOut.toFixed(2) + '</p></div></div>';
        html += '<div class="card card-balance"><div class="card-glass"><div class="card-header"><div class="card-icon-wrapper icon-purple"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg></div><h3>अखेर शिल्लक</h3></div><p class="amount">\u20B9' + finalBalance.toFixed(2) + '</p></div></div>';
        html += '</div>';

        html += '<div class="table-scroll"><table><thead><tr><th>अ.क्र.</th><th>तपशील / नाव</th><th>तारीख</th><th>माध्यम</th><th>जमा (₹)</th><th>खर्च (₹)</th><th>शिल्लक (₹)</th></tr></thead><tbody>' + rows + '</tbody></table></div>';
    ...
    async function viewPDFYear(year, btn) {
        ...
        const originalText = btn ? btn.innerHTML : 'अहवाल पहा';
    ...
    async function downloadPDFYear(year, btn) {
        if (!isYearVisible(year)) return;
        const originalText = btn ? btn.innerHTML : 'अहवाल डाउनलोड करा';
        if (btn) { btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> कृपया थांबा...'; btn.disabled = true; }
        await new Promise(r => setTimeout(r, 50));
        try {
            let entries = [];
    ...
    function formatDate(dateStr, shortYear = false) {
        if (!dateStr) return '-';
        let d;
        ...
        if (shortYear) {
            return `<div class="date-dm" style="display:block; white-space:nowrap; font-weight:600; font-size:12px; line-height:1.2;">${day} ${month}</div><div class="date-y" style="display:block; white-space:nowrap; font-weight:600; font-size:12px; line-height:1.2; margin-top:2px;">${fullYear}</div>`;
        } else {
    ...
    async function generatePDFHTML(entries, year, logoDataURL) {
        ...
        const rows = entries.map((entry, index) => {
            ...
            const isOnline = (entry.mode === 'Online' || entry.mode === 'ऑनलाइन');
            const modeClass = isOnline ? 'online' : 'cash';
            const modeText = isOnline ? 'ऑनलाइन' : 'कॅश';
            return '<tr><td class="sr-no">' + (index+1) + '</td><td class="remark">' + entry.name + '</td><td class="date">' + formatDate(entry.date, true) + '</td><td class="mode ' + modeClass + '">' + modeText + '</td><td class="cash-in-cell">' + (entry.cash_in > 0 ? formatPDFCurrency(entry.cash_in) : '-') + '</td><td class="cash-out-cell">' + (entry.cash_out > 0 ? formatPDFCurrency(entry.cash_out) : '-') + '</td><td class="balance-cell">' + formatPDFCurrency(runningBalance) + '</td></tr>';
        }).join('');
    ...
    function renderPagination() {
        ...
        return `
            <div class="pdf-pagination">
                ...
                <span class="pdf-page-info">पेज ${uploadedPDFsPage + 1}/${totalPages}</span>
    ...
    function loadLogoForCover() {
        return new Promise((resolve) => {
            ...
            img.onload = function () {
                ...
                canvas.width = this.naturalWidth || 1024;
                canvas.height = this.naturalHeight || 1024;
...
<div class="cover-page">
  ...
  <div class="cover-content">
    ...
    <div class="cover-mandal-name">${s.orgName.includes('शिवसृष्टी') ? '<div style="font-size:1.4em; color:#ffd700; text-shadow:0 0 25px rgba(255,140,0,1); line-height:1.2; margin-bottom:12px;">शिवसृष्टी</div><div style="font-size:0.9em; color:#ffcc00; margin-bottom:5px;">हिंदू तरुण मित्र मंडळ</div>' : s.orgName}</div>
  ...
  <div class="cover-footer">
    ...
    <p>Website Developed By Dhananjay Ranate</p>
  </div>
</div></body></html>`;
    }
